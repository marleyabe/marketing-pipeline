...
class TestInitializeSchemas:
    ...
    def test_creates_bronze_google_ads_demographics_table(self, memory_connection):
        ...
        columns = memory_connection.execute(
            "SELECT column_name FROM information_schema.columns "
            "WHERE table_schema = 'bronze' AND table_name = 'google_ads_demographics_raw' "
            "ORDER BY ordinal_position"
        ).fetchall()
        column_names = [c[0] for c in columns]
        ...
        assert "customer_id" in column_names
        assert "campaign_id" in column_names
        assert "ad_group_id" in column_names
        assert "dimension_type" in column_names
        assert "dimension_value" in column_names
        assert "impressions" in column_names
        assert "clicks" in column_names
        assert "spend" in column_names
        assert "conversions" in column_names
